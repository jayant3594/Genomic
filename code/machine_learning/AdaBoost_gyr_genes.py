...
'''
The code is shown only for one feature file ("gyr_gene_mutation.csv"), 
which contains mutations in all position of gyrB and gyrA
'''
class Data():
...
if __name__ == '__main__':
    ...
    data = Data()
    csv_path = "C:\\...\\mutation_data\\gyr_gene_mutation.csv"
    ...
    #*****************************   AdaBoost DT: Gyr gene data   *****************************

    classifier = AdaBoostClassifier(n_estimators=80,learning_rate=0.075)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost Gyr gene data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost DT Gyr gene data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost DT Gyr gene data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")
    
    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_DT_Gyr_gene'])
    AUC.to_csv((r'C:\\...\\AdaBoost_DT_Gyr_gene.csv'), index=False)
    
    plot = get_plots(plot, alogo_gene_name = ' Adaboost DT: Gyr gene data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_DT_Gyr_gene.png',dpi=200)
    plot.show()

    # *****************************   AdaBoost LR: Gyr gene data   ***************************** #

    lgr = LogisticRegression(solver='lbfgs',random_state=42)
    classifier = AdaBoostClassifier(n_estimators=120, base_estimator=lgr
                                ,learning_rate=1) # Selecting LR as a base estimator
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost LR Gyr gene data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost LR Gyr gene data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost LR Gyr gene data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")
    
    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_LR_Gyr_gene'])
    AUC.to_csv((r'C:\\...\\AdaBoost_LR_Gyr_gene.csv'), index=False)
    
    plot = get_plots(plot, alogo_gene_name = ' Adaboost LR: Gyr gene data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_LR_Gyr_gene.png',dpi=200)
    plot.show()

    # *****************************  AdaBoost SVM Linear kernel: Gyr gene data  *****************************
    ...
    classifier = AdaBoostClassifier(n_estimators=20, base_estimator=svc,
                                    learning_rate=0.5)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost SVM-Linear Gyr gene data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost SVM-Linear Gyr gene data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost SVM-Linear Gyr gene data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")
    
    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_SVM-Linear_Gyr_gene'])
    AUC.to_csv((r'C:\\...\\AdaBoost_SVM-Linear_Gyr_gene.csv'), index=False)
    
    plot = get_plots(plot, alogo_gene_name = ' Adaboost SVM-Linear: Gyr gene data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_SVM-Linear_Gyr_gene.png',dpi=200)
    plot.show()

    # ***************************** AdaBoost SVM rbf kernel: Gyr gene data  ***************************** #
    # Selecting SVM as a base estimator
    svc = SVC(probability=True, gamma='auto', kernel='rbf',random_state = 42) # Selecting kernel as 'rbf'
    classifier = AdaBoostClassifier(n_estimators=20, base_estimator=svc,
                                    learning_rate=0.25)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost SVM-rbf Gyr gene data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost SVM-rbf Gyr gene data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost SVM-rbf Gyr gene data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")
    
    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_SVM-rbf_Gyr_gene'])
    AUC.to_csv((r'C:\\...\\AdaBoost_SVM-rbf_Gyr_gene.csv'), index=False)
    
    plot = get_plots(plot, alogo_gene_name = ' Adaboost SVM-rbf: Gyr gene data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_SVM-rbf_Gyr_gene.png',dpi=200)
    plot.show()
#-----------------------------------------------     ***     -----------------------------------------------------#