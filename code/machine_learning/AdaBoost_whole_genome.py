...
'''
The code is shown only for one feature file ("whole_genome.csv"), 
which contains genomic as well as intergenic mutation data
'''
class Data():
    '''
...
if __name__ == '__main__':
    ...
    data = Data()
    # Uploading data
    csv_path = "C:\\...\\whole_genome.csv"
    whole_gene_df = data.get_data(csv_path)
    X = data.get_features(whole_gene_df,scaling=False, column_name=['Isolate_no','Class'])
    print("Shape of feature matrix : ",X.shape)
    y = data.get_targets(whole_gene_df, column_name=['Class'])
    print("Shape of target matrix : ",y.shape)

    #*****************************   AdaBoost DT: Whole gene data   *****************************#
    
    classifier = AdaBoostClassifier(n_estimators=80,learning_rate=0.1)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost DT Whole genome data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost DT Whole genome data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost DT Whole genome data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")

    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_DT_whole_genome'])
    AUC.to_csv((r'C:\\...\\AdaBoost_DT_whole_genome.csv'), index=False)

    plot = get_plots(plot, alogo_gene_name = ' Adaboost DT: Whole genome data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_DT_whole_genome.png',dpi=200)
    plot.show()

    # *****************************   AdaBoost LR: Whole gene data   ***************************** #
    ...
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost LR Whole genome data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost LR Whole genome data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost LR Whole genome data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")

    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_LR_whole_genome'])
    AUC.to_csv((r'C:\\...\\AdaBoost_LR_whole_genome.csv'), index=False)

    plot = get_plots(plot, alogo_gene_name = ' Adaboost LR: Whole genome data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_LR_whole_genome.png',dpi=200)
    plot.show()

    # *****************************  AdaBoost SVM Linear kernel: Whole gene data  *****************************
    ...
    classifier = AdaBoostClassifier(n_estimators=20, base_estimator=svc,
                                    learning_rate=0.75)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost SVM-Linear Whole genome data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost SVM-Linear Whole genome data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost SVM-Linear Whole genome data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")

    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_SVM-Linear_whole_genome'])
    AUC.to_csv((r'C:\\...\\AdaBoost_SVM-Linear_whole_genome.csv'), index=False)

    plot = get_plots(plot, alogo_gene_name = ' Adaboost SVM-Linear: Whole genome data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_SVM-Linear_whole_genome.png',dpi=200)
    plot.show()

    # ***************************** AdaBoost SVM rbf kernel: Whole gene data  ***************************** #
    ...
    classifier = AdaBoostClassifier(n_estimators=20, base_estimator=svc,
                                    learning_rate=1)
    aucs, accuracy_list, confusion_mat, plot = cross_validation(X,y,classifier,plt)
    print("\nMean Accuracy of Adaboost SVM-rbf Whole genome data: ", np.array(accuracy_list).mean())
    print("Mean Area under the curve (AUC) for Adaboost SVM-rbf Whole genome data : ",np.array(aucs).mean())
    print("Mean Standard deviation of AUC for Adaboost SVM-rbf Whole genome data : ",np.std(aucs))
    print('\nConfusion Matrix :')
    print(*confusion_mat.items(),sep = "\n")

    AUC = pd.DataFrame(data=aucs, columns = ['AdaBoost_SVM-rbf_whole_genome'])
    AUC.to_csv((r'C:\\...\\AdaBoost_SVM-rbf_whole_genome.csv'), index=False)

    plot = get_plots(plot, alogo_gene_name = ' Adaboost SVM-Linear: Whole genome data',initialization = False)
    plot.savefig(r'C:\\...\\AdaBoost_SVM-rbf_whole_genome.png',dpi=200)
    plot.show()
